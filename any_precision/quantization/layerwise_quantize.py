...
from .layerwise_uniform import train_uniform
@torch.no_grad()
...
def seed_layer(
    ...
    for m_idx in range(len(layer_modules)):
        ...
        if solver == "uniform":
            # Uniform LNQ + GuidedQuant. Ignores SqueezeLLM init_labels/centroids
            # (uniform grid is built from W). symmetric flag rides flexnu_kwargs.
            _uk = flexnu_kwargs or {}
            labels, C, log_dict = train_uniform(
                reshaped_module_weight, init_labels, init_centroids,
                module_hessian,
                seed_bit=seed_bit,
                num_iterations=num_iterations, cd_cycles=cd_cycles,
                symmetric=bool(_uk.get("uniform_symmetric", True)),
            )
        elif solver == "lnqbopt":
            ...
            _fk = {k: v for k, v in (flexnu_kwargs or {}).items()
                   if not k.startswith("bopt_") and not k.startswith("uniform_")}
